...
def service_ids(gtfs_root_dir, messages):
    service_ids = set()
    if not (gtfs_root_dir / 'calendar.txt').exists() and not (
            gtfs_root_dir / 'calendar_dates.txt').exists():
    ...
    for line in utils.read_csv_file(gtfs_root_dir, schema.CALENDAR, messages):
        ...
        if line.service_id in service_ids:
            line.add_error(DUPLICATE_SERVICE_ID,
                           f'service_id: {line.service_id}')

        service_ids.add(line.service_id)

    for line in utils.read_csv_file(gtfs_root_dir, schema.CALENDAR_DATES,
                                    messages):
        if not line.service_id:
