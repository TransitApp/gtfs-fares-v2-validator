...
def check_areas(path, line, areas, unused_areas, messages):
    if line.is_symmetrical and line.is_symmetrical not in {'0', '1'}:
        messages.add_error(INVALID_IS_SYMMETRICAL_LEG_RULES, line.line_num_error_msg)
...
def check_distances(line, messages):
    if line.distance_type and line.distance_type not in {'0', '1'}:
        messages.add_error(INVALID_DISTANCE_TYPE, line.line_num_error_msg)

    if line.min_distance:
        try:
            dist = float(line.min_distance)
            if dist < 0:
                messages.add_error(NEGATIVE_MIN_DISTANCE, line.line_num_error_msg)
        except ValueError:
            messages.add_error(INVALID_MIN_DISTANCE, line.line_num_error_msg)
    ...
    if (not line.min_distance and not line.max_distance) and line.distance_type:
        messages.add_error(DISTANCE_TYPE_WITHOUT_DISTANCE, line.line_num_error_msg)
