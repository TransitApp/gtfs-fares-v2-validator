...
def timeframes(gtfs_root_dir, messages):
    timeframes = set()
    for line in read_csv_file(gtfs_root_dir, schema.TIMEFRAMES, messages):
...
def fare_products(gtfs_root_dir, gtfs, unused_timeframes, messages):
    ...
    for line in read_csv_file(gtfs_root_dir, schema.FARE_PRODUCTS, messages):
        ...
        timeframe_exists = check_linked_id(line, 'timeframe_id',
                                           gtfs.timeframe_ids)
...
def fare_transfer_rules(gtfs_root_dir, gtfs, messages):
    ...
    for line in read_csv_file(gtfs_root_dir, schema.FARE_TRANSFER_RULES,
        ...
        if line.filter_fare_product_id and line.filter_fare_product_id not in gtfs.linked_entities_by_fare_product:
            line.add_error(NONEXISTENT_FILTER_FARE_PRODUCT_ID)
