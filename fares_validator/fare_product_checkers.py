...
def check_linked_fp_entities(line, rider_categories, rider_category_by_fare_container, linked_entities_by_fare_product,
    ...
    if line.fare_container_id:
        ...
        if line.fare_container_id not in rider_category_by_fare_container:
            messages.add_error(NONEXISTENT_FARE_CONTAINER_ID, line.line_num_error_msg, 'fare_products.txt')
...
def check_durations_and_offsets(line, messages):
    ...
    if line.duration_unit and line.duration_unit not in {'0', '1', '2', '3', '4', '5', '6'}:
        messages.add_error(INVALID_DURATION_UNIT, line.line_num_error_msg)
    ...
    if line.duration_amount:
        ...
        if not line.duration_type:
            messages.add_error(DURATION_WITHOUT_TYPE, line.line_num_error_msg)
    else:
        if line.duration_type:
            messages.add_error(DURATION_TYPE_WITHOUT_AMOUNT, line.line_num_error_msg)
        if line.duration_unit:
            messages.add_error(DURATION_UNIT_WITHOUT_AMOUNT, line.line_num_error_msg)

    if line.offset_unit and line.offset_unit not in {'0', '1', '2', '3', '4', '5', '6'}:
        messages.add_error(INVALID_OFFSET_UNIT, line.line_num_error_msg)
    ...
    if line.offset_amount:
        ...
        if line.offset_unit:
            messages.add_error(OFFSET_UNIT_WITHOUT_AMOUNT, line.line_num_error_msg)
